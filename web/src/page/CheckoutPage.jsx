...
export function CheckoutPage({ selectedFilm, showAlert, done }) {
  ...
  useEffect(() => {
    if (selectedSession) {
      axios.get(`http://localhost:3000/bookings?filmId=${selectedFilm.id}&session=${selectedSession.session}`)
        .then(response => {
          ...
          <Grid item xs={12} display="grid" gridTemplateColumns="3fr 1fr" gridTemplateRows="repeat(1fr, auto)" gap={1}>
            ...
            {selectedSession &&
              <Paper sx={{ gridRow: '2 / 4', padding: "1rem" }}>
                ...
                <Seats lockSeats={bookingSeats} selectedSeats={selectedSeats} toogleSeatClick={toogleSeatClick} />
              </Paper>
            }
                    ...
                    <Box>
                      ...
                      <List sx={{ width: '100%', padding: "1rem", bgcolor: 'background.paper' }}>
                        {selectedSeats.map(seat => (
                          ...
                          >
                            <ListItemText primary={`Assento ${seat} (${formatter.format(selectedSession?.value)})`} />
                          </ListItem>
                        ))}
                      </List>
                      <Typography>Total: {formatter.format((selectedSeats.length * selectedSession?.value) | 0)}</Typography>
                    </Box>
...
function Seats({ lockSeats = [], selectedSeats = [], toogleSeatClick }) {
  return (
